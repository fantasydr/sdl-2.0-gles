...
#if SDL_VIDEO_DRIVER_RPI && SDL_VIDEO_OPENGL_ES
...
void *
RPI_GLES_GetProcAddress(_THIS, const char *proc)
{
    SDL_Log("RPI_GLES_GetProcAddress");
...
void
RPI_GLES_UnloadLibrary(_THIS)
{
    SDL_Log("RPI_GLES_UnloadLibrary");
...
{
    SDL_Log("RPI_GLES_LoadLibrary %s", path);
    void *handle;
    int dlopen_flags;
    ...
    _this->gles_data->egl_display =
        _this->gles_data->eglGetDisplay(EGL_DEFAULT_DISPLAY); // (NativeDisplayType) data->display);
...
// XVisualInfo *
// RPI_GLES_GetVisual(_THIS, Display * display, int screen)
void
RPI_GLES_SetupAttributes(_THIS)
{
    ...
    if (!_this->gl_config.driver_loaded) {
        ...
        if (RPI_GLES_LoadLibrary(_this, NULL) < 0) {
            return; // NULL;
        }
    }
...
{
    SDL_Log("RPI_GLES_CreateContext");
    ...
    if (RPI_GLES_MakeCurrent(_this, window, context) < 0) {
        RPI_GLES_DeleteContext(_this, context);
        return NULL;
    }
...
int
RPI_GLES_MakeCurrent(_THIS, SDL_Window * window, SDL_GLContext context)
{
    SDL_Log("RPI_GLES_MakeCurrent");
    int retval;
...
int
RPI_GLES_SetSwapInterval(_THIS, int interval)
{
    SDL_Log("RPI_GLES_SetSwapInterval");
...
int
RPI_GLES_GetSwapInterval(_THIS)
{
    SDL_Log("RPI_GLES_GetSwapInterval");
    if (_this->gles_data->egl_active != 1) {
        SDL_SetError("OpenGL ES context not active");
        return -1;
    }
...
{
    // SDL_Log("RPI_GLES_SwapWindow");
    _this->gles_data->eglSwapBuffers(_this->gles_data->egl_display,
                                     _this->gles_data->egl_surface);
}

void
RPI_GLES_DeleteContext(_THIS, SDL_GLContext context)
{
    SDL_Log("RPI_GLES_DeleteContext");
